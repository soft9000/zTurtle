...
/**
 * This class is an implementation of a graphics canvas window for TG. All
 * graphics (pixels painted) are maintained in an Image for redisplay in the
 * event of resizing.
 * <p>
 * Other features of TGCanvas are:
 * <p>
 * - the coordinate space is the traditional mathematics; [0,0] is in the middle
 * of the graphics window.
 * <p>
 * - KeyListener method invocations are handled and may be passed on.
 * <p>
 * - MouseListener.mouseReleased() and MouseMotionListener.mouseMoved() are
 * captured and can be passed on.
 * <p>
 * - higher-level functionality (i.e., flood-fill provided).
 * <p>
 * - in-memory Image maintained with buffered operations to improve graphics
 * display performance.
 * <p>
 * @author Guy Haas
 */
public class TGCanvas extends Component
    ...
    /**
     * The SETPENCOLOR command's input is a built-in color number or an RGB
     * value. Built-in colors numbers are in range of 0-31 with 0 for black.
     */
    ...
    /**
     * The maximum value for a Logo color which require conversion to RGB colors
     * used by Java.
     */
    public static final int MAX_LOGO_COLOR = 31;

    /**
     * All derivations of Logo have a basic, minimum set of colors that are
     * specified by numbers. jLogo supports a set of sixteen, compatible with
     * Brian Harvey's Berkeley Logo.
     */
    ...
    * propagating mouse stuff
    ...
    // ------------- --------- -------
    /**
     * Invoked when this Component gets focus. Note that this has happened, that
     * the graphics canvas now gets keyboard and mouse events.
     */
    ...
    * component - the MouseEvent is given to it.
    ...
    // ------------------- --------- -------
    /**
     * Invoked when a mouse button is held down on TGCanvas and the mouse is
     * moved.
     */
    ...
    /**
     * Invoked when the mouse is moved around on the TGCanvas without any
     * buttons in a depressed state.
     */
    ...
    * print an error message to console tying it to this class
     ...
     * <p>
     * Something has occured that requires updating the graphics canvas. As an
     * example, TGGraphicsOps have been queued to be performed, or a Sprite
     * whose image is being displayed has moved or rotated, or the AWT has
     * decided we need to redraw at least some subset of our pixels, e.g.,
     * partially covered stuff (by some other application on the desktop) has
     * moved/gone away, etc...
     */
     ...
     * is specified by x and y. The new size is specified by width and height.
     */
     ...
     * <p>
     * Overridden in TGCanvas to eliminate Component.update()'s invocation of
     * Graphics.clear().
     */
    ...
    // ------ -------
    /**
     * Append a graphics operation, a TGGraphicsOp object, to the queue of
    ...
    /**
     * Add an object which implements TGKeyHandler to the list of those that
     * want a callback when a key is pressed while the graphics canvas has
     * focus. The object's keyPressed() method is invoked.
     */
    ...
    /**
     * Add an object which implements TGMouseHandler to the list of those that
     * want a callback. A callback is performed when the graphics canvas has
     * focus and
     * <ul>
     * <li>
    ...
    /**
     * Return the background Logo color number. Values zero through fifteen
     * (inclusive) are fixed Logo colors (black, blue, etc...) other values are
     * RGB numbers, with or without alpha values.
     */
    ...
    /**
     * Return the contents of the graphics canvas overlaid with existing Sprite
     * images as a composite BufferedImage.
     */
    ...
    /**
     * Return an array of pixel RGB values. A rectangular area of the graphics
     * canvas defined by a top-left corner (in TurtleSpace coordinates) and its
     * width and height.
     * <p>
     * Since all drawing operations are queued to be performed by the paint()
     * method, we must wait for all outstanding operations to complete before we
     * grab pixels.
     */
    ...
    /**
     * Convert a Logo color to a Java Color object. If the alpha component is
     * zero it is replaced with 0xFF - assumed to be an opaque color.
     */
    ...
    /**
     * Return the TurtleSpace x-coordinate of the mouse when it was last
     * clicked.
     * <p>
     * @see #mousey
     */
    ...
    /**
     * Return the TurtleSpace y-coordinate of the mouse when it was last
     * clicked.
     * <p>
     * @see #mousex
     */
    ...
    /**
     * Remove an object from TGCanvas' list of those wanting their keyPressed()
     * method invoked when TGCanvas receives this Event.
     */
    ...
    /**
     * Remove an object from TGCanvas' list of those wanting their
     * mouseClicked() and mouseMoved() methods invoked when we receive these
     * Events.
     */
    ...
    /**
     * Remove the specified Sprite from the list of sprites whose images are
     * displayed on the graphics canvas.
     */
